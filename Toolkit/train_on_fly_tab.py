...
class TrainOnFlyTab(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        # Models Directory Selection (initially hidden)
    ...
    def record_audio(self):
        try:
            duration = float(self.recording_duration.get())
            fs = int(self.sampling_rate.get())  # Use user-specified sampling rate
    ...
    def process_audio(self, audio, fs):
        ...
        if self.remove_silence.get():
            # Use librosa's effects.trim
            audio, _ = librosa.effects.trim(audio, top_db=20)
        
        if self.reduce_noise.get():
            # Use noisereduce to reduce background noise
    ...
    def segment_audio(self, audio, fs):
        ...
        for i in range(0, len(audio), segment_length):
            ...
            if len(segment) == segment_length:  # Only keep full-length segments
